...
$fdt = docker exec a2o-adabas sh -lc "adarep db=1 fdt file=12" 2>&1
...
foreach ($f in $newFields) {
    ...
    $out = docker exec a2o-adabas sh -c $script
...
foreach ($f in @("TRAFFINE.fdt", "TRAFFINE.fdu", "create-fine-file.sh")) {
    docker cp (Join-Path $naturalDir $f) "a2o-adabas:/tmp/$f" | Out-Null
    if ($LASTEXITCODE -ne 0) { Write-Error "docker cp $f failed"; exit 1 }
}
$out = docker exec a2o-adabas sh /tmp/create-fine-file.sh
...
$out = docker exec a2o-natural sh /poc/natural/run-seed.sh
